...
public class LoginPengelolaController {
    @FXML
    ...
    private void handleLoginButton(ActionEvent event) {
        ...
        if (username.equals("topa08") && password.equals("1234")) { //set username & password
            try {
    ...
    private void handleBackButton(ActionEvent event){
        try {
            Parent root = FXMLLoader.load(getClass().getResource("/resource/welcome.fxml")); // load file fxml
            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.setScene(new Scene(root));
            stage.show();
